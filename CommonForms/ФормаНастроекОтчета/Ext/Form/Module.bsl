...
Процедура ПриЗакрытии()
	Если Не ФормаЗакрываетсяСПараметрами
...
Процедура Подключаемый_СписокСПодбором_ПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Иначе
		...
		Если ТекущаяСтрока = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ЭлементСпискаВФорме = СписокЗначенийВФорме.НайтиПоИдентификатору(ТекущаяСтрока);
	КонецЕсли;
	...
	Иначе
		ЭлементСпискаВФорме.Представление = Строка(Значение);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВсеНастройки" Тогда
		...
		Если Элементы.ВсеНастройки.Пометка Тогда
			...
			Заголовок = НСтр("ru = 'Все настройки отчета'") + " """ + Заголовок + """";
		Иначе
		...
		Конецесли;
		Элементы.ГруппаДругиеВариантыОтчета.Заголовок = НастройкиОтчета.Наименование + " (" + НСтр("ru = 'варианты отчета'") + "):";
	КонецЕсли;
...
Процедура БыстрыеНастройкиУдалитьСтарыеЭлементыИКоманды(ПараметрыЗаполнения)
	...
	ДобавитьПодчиненныеЭлементы(УдаляемыеЭлементы, Элементы.СтраницаГлавноеКолонка1.ПодчиненныеЭлементы);
	ДобавитьПодчиненныеЭлементы(УдаляемыеЭлементы, Элементы.СтраницаГлавноеКолонка2.ПодчиненныеЭлементы);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения)
	...
	ГруппыВывода.Вставить("Основные", Новый Структура("Порядок, Размер", Новый СписокЗначений, 0));
	ГруппыВывода.Вставить("ДополнительныеШапка", Новый Структура("Порядок, Размер", Новый СписокЗначений, 0));
	ГруппыВывода.Вставить("ДополнительныеПодвал", Новый Структура("Порядок, Размер", Новый СписокЗначений, 0));
	...
	Пока Истина Цикл
		...
		Для Каждого ПользовательскаяНастройка Из НаборПользовательскихНастроек Цикл
			...
			Иначе
				...
				Иначе // Раздел найден. Вывод в группу на закладке "Дополнительно":
					...
					Если ГруппаВывода = Неопределено Тогда
						ГруппаВывода = Новый Структура("Порядок, Размер", Новый СписокЗначений, 0);
			...
			Если СвойстваНастройки.Шаблон = "ТолькоФлажокИспользование" Или СвойстваНастройки.Шаблон = "ТолькоФлажокЗначения" Тогда
				...
				// Флажок.
				ИспользованиеОтбора = Элементы.Добавить(ФлажокИмя, Тип("ПолеФормы"), Элементы.СтраницаНеОтсортированное);
			...
			ИначеЕсли СвойстваНастройки.Шаблон = "СвязьСКомпоновщиком" Тогда
				...
				//ИменаОсновныхРеквизитовФормы.Вставить(ИдентификаторЭлемента, Путь + ".Значение");
				...
				Если СвойстваНастройки.ФлажокИспользование Тогда
					...
					Группа = Элементы.Добавить(ГруппаИмя, Тип("ГруппаФормы"), Элементы.СтраницаНеОтсортированное);
				...
				Иначе
					...
					Группа = Элементы.СтраницаНеОтсортированное;
			...
			ИначеЕсли СвойстваНастройки.Шаблон = "СтандартныйПериод" Тогда
				...
				// Основная Группа
				ГруппаСтрок = Элементы.СтраницаНеОтсортированное;
	...
	ГруппаВывода = ГруппыВывода.Основные;
	ТекущаяГруппа = Элементы.СтраницаГлавноеКолонка1;
	ОсталосьМеста = ГруппаВывода.Размер/2;
	Для Каждого ЭлементСписка Из ГруппаВывода.Порядок Цикл
		Если ОсталосьМеста > 0 И ОсталосьМеста < ЭлементСписка.Значение/4 Тогда // Текущий шаг больше половины оставшегося места.
			ТекущаяГруппа = Элементы.СтраницаГлавноеКолонка2;
			ОсталосьМеста = 0;
		КонецЕсли;
		Элементы.Переместить(Элементы[ЭлементСписка.Представление], ТекущаяГруппа);
		Если ОсталосьМеста > 0 Тогда
	...
	Для Каждого ЭлементСписка Из ГруппыВывода.ДополнительныеШапка.Порядок Цикл
		Элементы.Переместить(Элементы[ЭлементСписка.Представление], Элементы.СтраницаДополнительноШапка);
	КонецЦикла;
	
	Для Каждого ЭлементСписка Из ГруппыВывода.ДополнительныеПодвал.Порядок Цикл
		Элементы.Переместить(Элементы[ЭлементСписка.Представление], Элементы.СтраницаДополнительноПодвал);
	КонецЦикла;
	...
	Для Каждого ОписаниеЗакладки Из ГруппыВывода.ДополнительныеЗакладки Цикл
		...
		Колонка1Имя = "ГруппаНастроек_Колонка1_" + Строка(НомерДополнительнойГруппы);
		Колонка2Имя = "ГруппаНастроек_Колонка2_" + Строка(НомерДополнительнойГруппы);
		...
		Если ГруппаВывода.Размер > 0 Тогда
			...
			Колонка1 = Элементы.Добавить(Колонка1Имя, Тип("ГруппаФормы"), Колонки);
			Колонка1.Вид                 = ВидГруппыФормы.ОбычнаяГруппа;
			Колонка1.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
			Колонка1.Отображение         = ОтображениеОбычнойГруппы.Нет;
			Колонка1.ОтображатьЗаголовок = Ложь;
			
			Колонка2 = Элементы.Добавить(Колонка2Имя, Тип("ГруппаФормы"), Колонки);
			Колонка2.Вид                 = ВидГруппыФормы.ОбычнаяГруппа;
			Колонка2.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
			Колонка2.Отображение         = ОтображениеОбычнойГруппы.Нет;
			Колонка2.ОтображатьЗаголовок = Ложь;
			
			Отступ = Элементы.Добавить(ОтступИмя, Тип("ДекорацияФормы"), Колонка1);
			Отступ.Вид       = ВидДекорацииФормы.Надпись;
			Отступ.Заголовок = "    ";
			
			Для Каждого ПодчиненныйЭлемент Из ГруппаВывода.Порядок Цикл
				Элементы.Переместить(Элементы[ПодчиненныйЭлемент.Представление], Колонка2);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
